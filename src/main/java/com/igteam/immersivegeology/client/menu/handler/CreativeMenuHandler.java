...
public class CreativeMenuHandler {
	private static final ResourceLocation CEX_GUI_TEXTURES = new ResourceLocation(ImmersiveGeology.MODID,
			"textures/gui/creative_expansion.png");
	...
	public class CreativeMenuButton extends Button {
		...
		public void renderButton(int mouseX, int mouseY, float pticks) {
			...
			AbstractGui.blit(x, y, ((hovered || (IGItemGroup.getCurrentSubGroup().equals(group))) ? 29 : 47), 0, width,
					height, 256, 256);
			...
			if (hovered) {
				...
				// we divide the width by four to force the tooltip to the left, as it's on a
				// lower layer then the items in the creative menu.
				GuiUtils.drawHoveringText(tooltip, mouseX, mouseY, (mc.currentScreen.width / 4),
						mc.currentScreen.height, 80, mc.fontRenderer);
			}
		}
	}
}
