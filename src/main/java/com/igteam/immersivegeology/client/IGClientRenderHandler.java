...
public class IGClientRenderHandler implements ItemColor, BlockColor {
    ...
    public static void register(){
        for(Item i : IGRegistrationHolder.getItemRegistryMap().values().stream().map(DeferredHolder::get).toList()){
            if(i instanceof IGFlagItem){
                Minecraft.getInstance().getItemColors().register(INSTANCE, i);
            }
        }

        for(Block b : IGRegistrationHolder.getBlockRegistryMap().values().stream().map(DeferredHolder::get).toList()){
