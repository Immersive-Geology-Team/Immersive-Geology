...
public class NoiseGen {
	/**
	 * Noise generation seed. Minecraft world seed should be used for
	 * reproducibility.
	 */
	private long seed;

	/**
	 * Number of FastNoise functions to use. This will be the number of valuesin a
	 * NoiseTuple. Recommended: 2
	 */
	 ...
	 * @param numGenerators Number of noise values to calculate per block. This
	 *                      number will be the number of noise values in each
	 *                      resultant NoiseTuple. Increasing this will impact
	 *                      performance.
	 * @param yComp         y-compression factor
	 * @param xzComp        xz-compression factor
	 */
	public NoiseGen(World world, boolean isFastNoise, NoiseSettings noiseSettings, int numGenerators, float yComp,
			float xzComp) {
	 ...
	 * @param xzComp        xz-compression factor
	 */
	public NoiseGen(long seed, boolean isFastNoise, NoiseSettings noiseSettings, int numGenerators, float yComp,
			float xzComp) {
		this.seed = seed;
		this.noiseSettings = noiseSettings;
		this.numGenerators = numGenerators;
	 ...
	 * 
	 * @param blockPos  Position of any block in the column (the y-coordinate is
	 *                  ignored)
	...
	/**
	 * Generate NoiseTuples for a column of blocks, as a NoiseColumn. Only blocks at
	 * the top & bottom of each subChunk (as designated by subChunkSize) have noise
	 * values actually generated for them. Blocks in between have noise values
	 * estimated via bilinear interpolation.
	 *
	 * This function is currently unused, since such interpolation along the y-axis
	 * ends up reducing cave interconnectivity. It is kept here for possible future
	 * use.
	 * 
	 * @param blockPos     Position of any block in the column (the y-coordinate is
	 *                     ignored)
	 * @param minHeight    The bottom y-coordinate to start generating noise values
	 *                     for
	 * @param maxHeight    The top y-coordinate to stop generating noise values for
	...
	public NoiseColumn interpolateNoiseColumn(BlockPos blockPos, int minHeight, int maxHeight, int subChunkSize) {
		...
		// Calculate noise for every nth block in the column, using bilinear
		...
		for (startY = minHeight; startY <= maxHeight; startY += subChunkSize) {
			...
			for (int y = startY + 1; y < endY; y++) {
				...
				if (endY == maxHeight) {
					...
					startCoeff = IGLib.START_COEFFS[y - startY];
					endCoeff = IGLib.END_COEFFS[y - startY];
				}
	...
	/**
	 * Generate NoiseTuples for a cube of blocks. Only columns of blocks at the four
	 * corners of each cube have noise values calculated for them. Blocks in between
	 * have noise values estimated via a naive implementation of trilinear
	 * interpolation.
	 * 
	 * @param startPos  Position of any block in the starting corner column of the
	 *                  cube. This column must have x and z coordinates lower than
	 *                  that of endPos.
	 * @param endPos    Position of any block in the ending corner column of the
	 *                  cube. This column must have x and z coordinates higher than
	 *                  that of startPos.
	 * @param minHeight The bottom y-coordinate to start generating noise values for
	 * @param maxHeight The top y-coordinate to stop generating noise values for
	 * @return NoiseCube
	...
	public NoiseCube interpolateNoiseCube(BlockPos startPos, BlockPos endPos, int minHeight, int maxHeight) {
		...
		NoiseColumn noisesX0Z0 = generateNoiseColumn(new BlockPos(startX, 1, startZ), minHeight, maxHeight);
		NoiseColumn noisesX0Z1 = generateNoiseColumn(new BlockPos(startX, 1, endZ), minHeight, maxHeight);
		NoiseColumn noisesX1Z0 = generateNoiseColumn(new BlockPos(endX, 1, startZ), minHeight, maxHeight);
		NoiseColumn noisesX1Z1 = generateNoiseColumn(new BlockPos(endX, 1, endZ), minHeight, maxHeight);
		...
		for (int x = 1; x < subChunkSize - 1; x++) {
			startCoeff = IGLib.START_COEFFS[x];
			endCoeff = IGLib.END_COEFFS[x];
		...
		for (int x = 0; x < subChunkSize; x++) {
			for (int z = 1; z < subChunkSize - 1; z++) {
				startCoeff = IGLib.START_COEFFS[z];
				endCoeff = IGLib.END_COEFFS[z];
