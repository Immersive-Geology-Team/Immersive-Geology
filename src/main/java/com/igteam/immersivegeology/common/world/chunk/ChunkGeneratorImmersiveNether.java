...
public class ChunkGeneratorImmersiveNether extends ChunkGenerator<ImmersiveNetherGenSettings>
{
	...
	private final ImmersiveNetherBiomeProvider biomeProvider;
	private final ChunkDataProvider chunkDataProvider;

	private final HashMap<Biome, INoise2D> skyNoiseMap = new HashMap<Biome, INoise2D>();

	public ChunkGeneratorImmersiveNether(IWorld world, BiomeProvider provider,
										 ImmersiveNetherGenSettings settings)
	{
	...
	public void makeBase(IWorld worldIn, IChunk chunk)
	{
		...
		double[] baseSkyHeight = new double[16*16];
		...
		for(int x = 0; x < 16; x++)
		{
			for(int z = 0; z < 16; z++)
			{
				...
				for(Object2DoubleMap.Entry<IGBiome> entry : heightBiomeMap.object2DoubleEntrySet()) {
					...
					double height = weight * skyNoiseMap.get(entry.getKey()).noise(chunkX + x, chunkZ + z);
					skyHeight -= height;
				}
				baseSkyHeight [x+16*z] = skyHeight;
		...
		for(int x = 0; x < 16; x++)
		{
			for(int z = 0; z < 16; z++)
			{
				...
				double skyHeight = baseSkyHeight[x+16*z];
				boolean noGen = true;
						...
						if (b.getLbiome() == biome) {
							...
							for (int l = lc; l > 0; l--) {
								...
								if ((y <= (totHeight * l) / lc)
									...
									chunk.setBlockState(posSky, replaceBlock.getDefaultState()
		...
		// Height maps
		chunk.func_217303_b(Type.OCEAN_FLOOR_WG);
		chunk.func_217303_b(Type.WORLD_SURFACE_WG);
	}
...
//			{
//				float noise = 0;
//				int topYLevel = chunk.getTopBlockY(Type.WORLD_SURFACE_WG, x, z)+1;
...
//		}
	...
	public void makeBedrock(IChunk chunk, Random random,int rockY)
	{
		...
		for(BlockPos pos : BlockPos.getAllInBoxMutable(chunk.getPos().getXStart(), rockY, chunk.getPos().getZStart(),
				chunk.getPos().getXStart()+15, rockY, chunk.getPos().getZStart()+15))
		{
			if(flatBedrock)
			{
